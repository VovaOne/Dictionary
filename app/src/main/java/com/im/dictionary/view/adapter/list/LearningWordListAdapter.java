...
public class LearningWordListAdapter extends WordListAdapter {

    public LearningWordListAdapter(List<Card> cards) {
        super(cards);
    ...
    public Holder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == Card.OPEN) {
            ...
            vh = new ExpandedCardHolder.Builder()
                    ...
                    .createLearningExpandedCardHolder();
    ...
    private void initBroadcastReceivers() {
        LocalBroadcastManager.getInstance(App.getAppContext()).unregisterReceiver(cardAddedBroadcastReceiver);
        LocalBroadcastManager.getInstance(App.getAppContext()).registerReceiver(cardAddedBroadcastReceiver,
                new IntentFilter(ADDED_CARD_INTENT));
    ...
    protected BroadcastReceiver cardAddedBroadcastReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            Card card = (Card) intent.getSerializableExtra(WordDialog.ADDED_CARD_ID_EXTRA);
            cards.add(0, card);
    ...
    public static View getOpenedCard(ViewGroup parent) {
        return LayoutInflater.from(parent.getContext()).inflate(R.layout.word_card_expanded_learning, parent, false);
    }

    @Override
    public void reloadCards(Card.Type type) {
        this.cards = CARD_DAO_INSTANCE.getCardsByType(LEARNING);
        notifyDataSetChanged();
    }
}
